...
- **Only edit**: `Makefile`, `skills.toml`, `README.md`, `CLAUDE.md`, `.gitignore`

## How it works

1. `make clone` clones upstream repos into `repos/`
2. `make install` reads `skills.toml` and creates symlinks in `~/.claude/skills/`
3. `make update` pulls latest from all repos
4. `make clean` removes managed symlinks

## Adding a new skill source

1. Add the repo URL to `[repos]` in `skills.toml`
2. Add a `[skills.<repo-name>]` section with `paths` and optional `agents`
